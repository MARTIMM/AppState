...
An example of the logfile is shown here;
  ...
  Uppercase letters s and f mean that the log will be forced while
  ...
  2014-08-10
  ----------
  14:09:06 IS =LG 0537 Logger initialized
       980 IS =LG 0448 Logging started. Log level set to 'ERROR'. Starting new log
       980 IS =LG 0164 Log level changed from 'ERROR' into 'TRACE'
       981 Is =LG 1027 Tag 'MXT' added for module 'main'
       000 Is =LG 1027 Tag '=CL' added for module
             'AppState::Plugins::Feature::CommandLine'
       000 Is =PM 0395 Object CommandLine created
  14:09:07 Is =CL 0092 Option processing changed: bundling
       003 Is =CL 0245 Options processed
       004 Is =PM 0402 Object CommandLine retrieved
       004 Is =PM 0402 Object ConfigManager retrieved
...
AppState::Plugins::Feature::CommandLine(=CL) modules shown in the above log.
...
line number where the log call was issued.
  ...
  14:09:07 Ef 320 0036 Config root nor config hook into data is a hash reference. Returned
              an empty hash reference, perhaps no document selected
               Stack dump;
               0036 main
               0234 Test::Builder
               0239 Test::Builder
...
Get the loggers structure as a hash reference. There are two loggers defined
called 'AppState::Plugins::Feature::Log::File' and
'AppState::Plugins::Feature::Log::Screen' They are defined by code constants
C_LOG_LOGGERFILE and C_LOG_LOGGERSCREEN.
...
defined and the purpose and keys are shown in a table.
  ...
  log.screen    Show log to stderr   '%p{1}%m{chomp}%n'
...
error status will kill the program unless die_on_fatal(0) is called first.
  ...
  C_LOG_LLVLCHANGED   M_TRACE      Log level changed
  C_LOG_SCREENLOG     M_TRACE      Log to screen turned on or off
...
=head1 PROGRAM CODES
  ...
  C_LOG_LOGGERFILE                 The name used to set/get the file logger
                                   from Log::Log4perl.
  C_LOG_LOGGERSCREEN               The name used to set/get the screen logger
                                   from Log::Log4perl.
