...
There are two loggers initialized. One for file output and one for stderr
output. The catagories are 'AppState::Log::File' and 'AppState::Log::Stderr' 
defined by constants $log->C_LOG_LOGGERFILE and $log->C_LOG_LOGGERSTDERR resp.
The logger to write to the logfile can be controlled by do_append_log(),
...
Return the hash reference of the module tags table. Cannot be written to. The
method add_tag() will use _set_log_tag() to modify the hash.
The keys in the hash are the names of the modules and their values are the 3
letter tag used in the log output. The first entry is that of the log module;
...
#'AppState::Plugins::Feature::Log::Stderr' They are defined by code constants
#C_LOG_LOGGERFILE and C_LOG_LOGGERSTDERR.
#
...
error status will kill the program unless die_on_fatal(0) is called first.
  ...
  C_LOG_FILELOGLVL    M_TRACE      Log level for file logging changed
  C_LOG_STDERRLOGLVL  M_TRACE      Log level for stderr changed
...
=head1 PROGRAM CODES
  ...
  C_LOG_LOGGERFILE                 The name used to set/get the file logger
                                   from Log::Log4perl.
  C_LOG_LOGGERSTDERR               The name used to set/get the screen logger
                                   from Log::Log4perl.to send messages to stderr
