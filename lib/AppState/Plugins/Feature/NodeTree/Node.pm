package AppState::Plugins::Feature::NodeTree::Node;
...
extends 'AppState::Plugins::Feature::NodeTree::NodeRoot';
...
subtype 'AppState::Plugins::Feature::NodeTree::Node::ValidNodeChildType'
      => as 'Object'
      => where { ref($_) =~ m/AppState::Plugins::Feature::NodeTree::Node(Text)?/ }
      => message { 'Child object is not of proper type' };

has '+children' => (isa => 'ArrayRef[AppState::Plugins::Feature::NodeTree::Node::ValidNodeChildType]');

has attributes =>
    ( is                => 'rw'
    , isa               => 'ArrayRef[AppState::Plugins::Feature::NodeTree::NodeAttr]'
...
sub insert_above_node
{
  ...
  $node->parent->_remove_child($node)
    if ref $node->parent eq 'AppState::Plugins::Feature::NodeTree::Node';
  ...
  $self->parent->_replace_child( $self, $node)
    if ref $self->parent eq 'AppState::Plugins::Feature::NodeTree::Node';
...
sub add_attribute
{
  ...
  {
    my $av = $attrs{$an};
    my $node = AppState::Plugins::Feature::NodeTree::NodeAttr->new( name => $an, value => $av);
...
AppState::Plugins::Feature::NodeTree::Node - Node in the NodeTree
